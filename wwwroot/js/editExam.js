...
function appendQuestions(questions) {
	...
	$.each(questions, function (index, question) {
		...
		h_question =
			...
			//"Delete Question" +
			'<img src="images/icons/icons8-remove-16.png"/>'+
			"</button>" +
			"</th >" +
			"<td>" +
...
function addQuestion(questionB) {
	...
	$.ajax({
		...
		success: function (data, status, jqXHR) {
			alert(data.id);
			$('.btn-new-qustion').attr('data-id', data.id);
			$('.btn-remove-new').attr('data-id', data.id);
...
function appendQuestion() {
	...
	var h_question =
		...
		'class="btn btn-primary btn-sm btn-new-qustion" ' +
		'data-id="' +
		"new" +
		'">' +
		"Update New Qustion" +
